...
import { TrendingUp, TrendingDown, DollarSign, AlertOctagon, Utensils, ArrowUpRight } from "lucide-react";
import { 
...
export function MetricsClient({ orders, expenses, products }: { orders: any[], expenses: any[], products: any[] }) {
  ...
  const { filteredOrders, filteredExpenses, chartData, totals } = useMemo(() => {
     ...
     // Initialize days if not 'all' to fill empty days
     if (timeRange !== "all") {
                      ...
                      <div key={p.id} className="p-4 flex items-center justify-between hover:bg-slate-50">
                         ...
                         <div className="text-right">
                            <span className={`px-2 py-1 rounded text-xs font-bold ${
                               p.status === 'yellow' ? 'bg-yellow-100 text-yellow-700' : 'bg-green-100 text-green-700'
                            }`}>
